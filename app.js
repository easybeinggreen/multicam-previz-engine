...
// Single source of truth for vignette colour identity: one light/dark pair per vignette.
// VIGNETTE_COLORS (light) is used directly as the 2D floor-plan fill AND as the lightest
// stop of the 3D backdrop gradient — so the two views can't drift out of sync again.
// VIGNETTE_DARK_COLORS is the shadow end of the 3D gradient only.
...
const TARGETS = {
  ...
  "Vignette1-2": () => pt((V.V1 + V.V2) / 2, R_WALL + STAGE_MARK_DEPTH_M),
  "Vignette4-5": () => pt((V.V4 + V.V5) / 2, R_WALL + STAGE_MARK_DEPTH_M)
};
...
let items = [
  ...
  { id: 3, type: "prop", label: "Table", x: STAGE_MARKS.V3.x + 2, y: STAGE_MARKS.V3.y, z: 0, w: 1.4, d: 0.98, h: 0.9, color: "#FAC775" }
];
...
ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(rotation);
      ...
      const rw = Math.max(it.w * scale, 5), rd = Math.max((it.d || it.w) * scale, 5);
      ctx.fillStyle = it.color;
      ctx.fillRect(p.x - rw / 2, p.y - rd / 2, rw, rd);
      ctx.strokeStyle = "#fff"; ctx.lineWidth = it === dragging ? 2 : 0.5;
      ctx.strokeRect(p.x - rw / 2, p.y - rd / 2, rw, rd);
...
window.PrevizState = {
  ...
  VIGNETTE_DARK_COLORS: VIGNETTE_DARK_COLORS,
